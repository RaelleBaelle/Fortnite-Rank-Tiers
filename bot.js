...
# Constant
DISCORD_TOKEN = "NDQxNjMxMzkzMTY0ODIwNDgw.DczE-A.Pnp_IrDjZZtm8oE0n_WgbZx9Lbk"

FORTNITE_API_KEY = '0ed759e3-fc36-4b97-92b9-ad633293c0b0'

LISTE = ['Newbie', 'Casual', 'Bronze', 'Silver', 'Gold', 'Platinum', 'Diamond', 'Master', 'Grandmaster', 'SemiPro', 'Pro']
NEWBIE_B = 0.00
NEWBIE_E = 0.99
CASUAL_B = 1.00
CASUAL_E = 1.49
...
async def on_message(message):
    ...
    if message.content.startswith("/rank"):
        ...
        print(ratio)
        if ratio >= NEWBIE_B and ratio <= NEWBIE_E:
            ...
            msgRatio = "Next level: " + str(ratio) + "k/d  **→**  " + str(CASUAL_B) + "k/d"
            await client.send_message(message.channel, msgRatio)
            await client.send_message(message.channel, print_nextLvl(NEWBIE_B, NEWBIE_E, ratio))
            await client.add_roles(message.author, role) 
        elif ratio >= CASUAL_B and ratio <= CASUAL_E:
            role = discord.utils.get(message.server.roles, name=LISTE[1])
            msg = ("{0.author.mention} You have been ranked " + role.name).format(message)
            await client.send_message(message.channel, msg)
            msgRatio = "Next level: " + str(ratio) + "k/d  **→**  " + str(BRONZE_B) + "k/d"
            await client.send_message(message.channel, msgRatio)
            await client.send_message(message.channel, print_nextLvl(CASUAL_B, CASUAL_E, ratio))
            await client.add_roles(message.author, role) 
